...
export const loadAppState = createServerFn({ method: "GET" })
  ...
  .inputValidator((input) => loadInput.parse(input ?? {}))
  .handler(async ({ context }) => {
    ...
    return { state: stateSchema.parse(rows[0].state), version: Number(rows[0].version) || 1 };
  });
...
export const saveAppState = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (serialized.length > 2_000_000) throw new Error("App state exceeds the 2 MB safety limit");
